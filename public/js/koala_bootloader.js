		...
		*/
        ...
        conn.onopen = function() {
            conn.send(JSON.stringify({
                type: 'setup',
                cid: browserCid,
                alias: operators[browserCid].alias
            }));
        ...
        if (conn.readyState == 0) {
            setTimeout(function() {
                open_new_socket_connection(host, port, cid1, cid2, from, to, protocol, isIncoming, cb)
            }, 500)
        } else {
            conn.send(JSON.stringify({
                type: 'setup',
        ...
        worker.onmessage = function(event) {
             ...
             */
                ...
                if (!addedHtml[id]) {
                    ...
                    if (diocan != null || diocan != undefiend) {
                        diocan.innerHTML = html
                    } else {
                        console.log('si bugga tutto')
                    }
                    addedHtml[id] = true
                }
            }
        ...
        now.get_clusters = function(cb, index) {
            ...
            for (var i in operators) {
                c.push({
                    cid: i,
                    workers: {
                        length: operators[i].__workerCounter
                    },
